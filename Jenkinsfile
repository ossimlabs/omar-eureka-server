properties([
    ...
    [$class: 'GithubProjectProperty', displayName: '', projectUrlStr: 'https://github.com/ossimlabs/omar-eureka-server'],
...
)
{
  node(POD_LABEL){
    ...
    stage("Checkout branch")
    {
        ...
        GIT_TAG_NAME = "omar-eureka-server" + "-" + VERSION
    ...
    DOCKER_IMAGE_PATH = "${DOCKER_REGISTRY_PRIVATE_UPLOAD_URL}/omar-eureka-server"
        ...
        container('kubectl-aws-helm') {
            ...
            region: 'us-east-1'){
                if (BRANCH_NAME == 'master'){
                    ...
                    sh "kubectl config set-context dev --namespace=omar-eureka-server"
                    sh "kubectl rollout restart deployment/omar-wmts"   
                }
                else {
